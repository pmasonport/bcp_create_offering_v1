...
const inp = { width:"100%",boxSizing:"border-box",padding:"8px 10px",fontSize:13,color:g[900],border:"1px solid "+g[200],borderRadius:4,outline:"none",fontFamily:font,transition:"border-color 0.15s" };
const lbl = { display:"block",fontSize:13,fontWeight:500,color:g[700],marginBottom:6 };
  ...
  return (
    <button onClick={onClick} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{position:"relative",display:"flex",flexDirection:"column",alignItems:"flex-start",gap:8,padding:"16px 16px 14px",
        border:selected?"1.5px solid "+blue:"1px solid "+(hov?g[300]:g[200]),borderRadius:4,
        background:selected?"#F8FAFF":hov?g[50]:"#fff",cursor:"pointer",textAlign:"left",outline:"none",
        transition:"all 0.15s"}}>
      {selected&&<div style={{position:"absolute",top:10,right:10,width:18,height:18,borderRadius:"50%",background:blue,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff"}}><I name="check"/></div>}
      <div style={{color:selected?blue:g[400],transition:"color 0.15s"}}><I name={icon}/></div>
      <div>
        <div style={{fontSize:14,fontWeight:600,color:g[900]}}>{title}</div>
        <div style={{fontSize:13,color:g[500],marginTop:1}}>{desc}</div>
      </div>
    </button>
  );
}
...
function Dropdown({value,options,onSelect,onCreate,placeholder,createLabel}){
  ...
  return(
    <div ref={ref} style={{position:"relative"}}>
      ...
      {open&&(
        <div style={{position:"absolute",top:"calc(100% + 4px)",left:0,right:0,zIndex:50,background:"#fff",border:"1px solid "+g[200],borderRadius:4,boxShadow:"0 4px 12px rgba(0,0,0,0.08)",overflow:"hidden",maxHeight:240,overflowY:"auto"}}>
          {options.map(o=>(
...
function SectionQ({children,sub}){
  return <div style={{marginBottom:10}}>
    <div style={{fontSize:12,fontWeight:600,color:g[500],textTransform:"uppercase",letterSpacing:"0.05em"}}>{children}</div>
    {sub&&<div style={{fontSize:12,color:g[400],marginTop:3,lineHeight:1.4}}>{sub}</div>}
  </div>;
}
...
function PricingModelCard({selected,onClick,title,desc,children}){
  ...
  return(
    <button onClick={onClick} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{padding:"14px",border:selected?"1.5px solid "+blue:"1px solid "+(hov?g[300]:g[200]),borderRadius:6,
        background:selected?"#F8FAFF":hov?g[50]:"#fff",cursor:"pointer",textAlign:"left",outline:"none",
        transition:"all 0.15s",display:"flex",flexDirection:"column",gap:8,position:"relative"}}>
      {selected&&<div style={{position:"absolute",top:8,right:8,width:16,height:16,borderRadius:"50%",background:blue,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff"}}><I name="check"/></div>}
      <div style={{height:32,display:"flex",alignItems:"flex-end",gap:1}}>{children}</div>
      <div><div style={{fontSize:12,fontWeight:600,color:g[900]}}>{title}</div><div style={{fontSize:11,color:g[500],marginTop:1}}>{desc}</div></div>
    </button>
  );
}
      ...
      <div style={{border:"1px solid "+g[200],borderRadius:6,overflow:"hidden"}}>
        ...
        {tiers.map((tier,i)=>{const isF=i===0;const isL=i===tiers.length-1;return(
          <div key={i} style={{display:"grid",gridTemplateColumns:"70px 90px 1fr 90px 32px",gap:0,padding:"6px 10px",borderBottom:isL?"none":"1px solid "+g[100],alignItems:"center",fontSize:13,background:i%2===0?"#fff":g[50]}}>
            ...
            <span>{isL?<span style={{color:g[400],fontSize:12}}>&infin;</span>:<input type="number" value={tier.to||""} onChange={e=>{const v=parseInt(e.target.value)||0;upTier(i,{to:v});if(i+1<tiers.length)upTier(i+1,{from:v+1})}} style={{...inp,width:70,padding:"4px 6px",fontSize:12,fontFamily:mono}}/>}</span>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:5}}>
      {options.map(o=>{const sel=value===o.id;return(
        <button key={o.id} onClick={()=>onChange(o.id)}
          style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",border:sel?"1.5px solid "+blue:"1px solid "+g[200],borderRadius:6,background:sel?"#F8FAFF":"#fff",cursor:"pointer",textAlign:"left",outline:"none",transition:"all 0.15s",fontFamily:font}}>
          <div style={{width:18,height:18,borderRadius:"50%",border:sel?"5px solid "+blue:"2px solid "+g[300],background:"#fff",flexShrink:0,transition:"all 0.15s"}}/>
          <div style={{flex:1}}>
            <div style={{fontSize:13,fontWeight:600,color:g[900]}}>{o.label}</div>
            {o.desc&&<div style={{fontSize:12,color:g[500],marginTop:1}}>{o.desc}</div>}
          </div>
        </button>
      )})}
    </div>
...
export default function CatalogWizard() {
  ...
  const canP1=bName.trim()&&bSlug.trim()&&grp;
  const confirmP1=()=>{
    setP1Done(true);setP1Edit(false);
    if(isBundle&&plans.length===0)setPlans([{id:uid(),name:"",slug:"",slugEd:false,inherits:false}]);
    if(!isBundle&&entryType==="product")setPlans([{id:uid(),name:bName,slug:bSlug,slugEd:true,inherits:false}]);
    if(isBundle)scrollTo(p2Ref);
  };
    ...
    return(
      <div style={{padding:"16px 16px 20px",borderTop:"1px solid "+blue+"30",borderRadius:"0 0 6px 6px"}}>
        ...
        {pc.pricingModel==="BLOCK"&&(isOTP||isPUSub)&&<div style={{marginBottom:20}}>
          ...
          {pc.blockSize&&pc.blockPrice&&<div style={{padding:"8px 12px",background:g[50],borderRadius:4,fontSize:12,color:g[500],fontFamily:mono}}>{pc.blockSize} units = 1 block = ${pc.blockPrice} &middot; {parseInt(pc.blockSize)*2+1} units = 2 blocks = ${(parseFloat(pc.blockPrice)*2).toFixed(2)}</div>}
        </div>}
  ...
  return(
    <div style={{minHeight:"100vh",background:"#fff",fontFamily:font}}>
      <div style={{borderBottom:"1px solid "+g[200],padding:"12px 32px",display:"flex",alignItems:"center",gap:8,fontSize:13,fontWeight:500}}>
        <span style={{color:g[400]}}>Catalog</span><span style={{color:g[300]}}>/</span><span style={{color:g[900]}}>{crumb}</span>
      </div>

      <div style={{maxWidth:600,margin:"0 auto",padding:"24px 24px 160px"}}>
        {/* Reset */}
        <div style={{display:"flex",justifyContent:"flex-end",marginBottom:16}}>
          <button onClick={resetAll} style={{padding:"5px 12px",fontSize:12,fontWeight:500,border:"1px solid "+g[200],borderRadius:4,background:"#fff",cursor:"pointer",color:g[500],fontFamily:font,transition:"all 0.15s"}} onMouseEnter={e=>{e.currentTarget.style.borderColor=red;e.currentTarget.style.color=red}} onMouseLeave={e=>{e.currentTarget.style.borderColor=g[200];e.currentTarget.style.color=g[500]}}>Reset</button>
        </div>
        ...
        {!step0Done?(
          ...
          <Fade><button onClick={()=>{setStep0Done(false);setP1Done(false);setP1Edit(false)}} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"6px 12px",border:"1px solid "+g[200],borderRadius:4,background:g[50],cursor:"pointer",outline:"none",fontFamily:font,fontSize:12,fontWeight:500,color:g[600],marginBottom:28}} onMouseEnter={e=>e.currentTarget.style.borderColor=g[300]} onMouseLeave={e=>e.currentTarget.style.borderColor=g[200]}>
            <I name={entryType==="addon"?"puzzle":isBundle?"layers":"file"}/>{entryType==="addon"?"Add-on":isBundle?"Product bundle":"Standalone product"}<span style={{color:g[400],marginLeft:2}}>&middot; Change</span>
          </button></Fade>
        )}
            ...
            {p1Done&&!p1Edit?(
              <Fade><button onClick={()=>setP1Edit(true)} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 16px",border:"1px solid "+g[200],borderRadius:4,background:g[50],cursor:"pointer",outline:"none",marginBottom:32,textAlign:"left",fontFamily:font,transition:"border-color 0.15s"}} onMouseEnter={e=>e.currentTarget.style.borderColor=g[300]} onMouseLeave={e=>e.currentTarget.style.borderColor=g[200]}>
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  ...
                  <div><div style={{fontSize:14,fontWeight:600,color:g[900]}}>{bName}</div><div style={{fontSize:12,color:g[400],fontFamily:mono,marginTop:1}}>{bSlug} <span style={{color:g[300],margin:"0 4px"}}>&middot;</span> <span style={{fontFamily:font,color:g[500]}}>{grp}</span></div></div>
                </div>
            ...
            {canPaths&&<div ref={pathRef} style={{marginTop:36}}><Fade delay={40}>
              ...
              {showPaths&&<div>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:3}}>
                      {grpDef.items.map(path=>{const gi=paths.indexOf(path);const fp=plans.find(p=>p.id===path.from);const tp=plans.find(p=>p.id===path.to);if(!fp||!tp)return null;const isEd=editPath===gi;return(<div key={gi} onMouseEnter={()=>setHovPath(gi)} onMouseLeave={()=>setHovPath(null)}>
                        <button onClick={()=>setEditPath(isEd?null:gi)} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 12px",border:"1px solid "+(isEd?blue:hovPath===gi?g[300]:g[200]),borderRadius:isEd?"6px 6px 0 0":6,background:isEd?"#F8FAFF":hovPath===gi?g[50]:"#fff",cursor:"pointer",outline:"none",fontFamily:font,transition:"all 0.15s"}}>
                          <div style={{display:"flex",alignItems:"center",gap:6,fontSize:13}}><span style={{fontWeight:600,color:g[900]}}>{fp.name}</span><span style={{color:grpDef.color,fontSize:14}}>&rarr;</span><span style={{fontWeight:600,color:g[900]}}>{tp.name}</span></div>
                ...
                :<div style={{marginTop:8,padding:14,border:"1px solid "+g[200],borderRadius:6,background:g[50]}}>
                  <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:10}}><select value={npFrom} onChange={e=>setNpFrom(e.target.value)} style={{...inp,width:"auto",flex:1}}><option value="">From&hellip;</option>{named.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select><span style={{color:g[300],fontSize:14,fontWeight:600}}>&rarr;</span><select value={npTo} onChange={e=>setNpTo(e.target.value)} style={{...inp,width:"auto",flex:1}}><option value="">To&hellip;</option>{named.filter(p=>p.id!==npFrom).map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
        ...
        {p2Done&&entryType==="product"&&(
          <div ref={p3Ref}><Fade delay={60}><div>
            ...
            <Fade delay={40}><Cont onClick={()=>console.log("Continue to Phase 4",{plans,planConfig,paths})} label="Continue to features"/></Fade>
          </div></Fade></div>
        )}

        {p1Done&&!isBundle&&!p2Done&&entryType==="product"&&<Fade delay={60}><Cont onClick={confirmP2} label="Continue to pricing"/></Fade>}

        {p1Done&&entryType==="addon"&&<Fade delay={60}><div style={{padding:"24px",border:"1px dashed "+g[300],borderRadius:4,textAlign:"center"}}><p style={{fontSize:13,color:g[400],margin:0}}>Add-on configuration &mdash; coming in Build 2.</p></div></Fade>}
      </div>
    </div>
  );
}
